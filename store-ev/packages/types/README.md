# Store Enchufe Virtual `types`

This package contains all `types` used by the Store Enchufe Virtual storefront.

You can install it and use them to build your own store data and configuration files.

To build store-ev data you'll probably need to write a script that converts data from your source to the store-ev data format.
...
The store-ev data are composed of a set of 8 JSON files usually located at `/data/json`.
...
Here below you can find a working example with Node.js 18 that converts a list of products coming from an external service into the store-ev `products.json`.
...
(async () => {
  ...
  const storeEvProducts: RawDataProduct[] = remoteProducts.map(
  ...
  await writeFile('products.json', JSON.stringify(storeEvProducts, undefined, 2))

  console.log(storeEvProducts)
