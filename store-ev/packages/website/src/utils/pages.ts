...
const getProductGridPageComponent = async (rawData: RawDataProductGrid, localeCode: string): Promise<ProductGridPageComponent> => {
  ...
  const products = rawData.skus
    .map(sku => {
      try {
        return getProductWithVariants(sku, localeCode, rawDataProducts)
      } catch (error) {
        console.warn(`Page component product not found for sku: ${sku}`)
        return null
      }
    })
    .filter((product): product is LocalizedProductWithVariant => product !== null)
