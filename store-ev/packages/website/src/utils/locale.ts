...
export function parseLocaleCode(localeCode: string | string[] | undefined) {
  const locale = Array.isArray(localeCode) ? localeCode[0] : localeCode

  if (!locale || typeof locale !== 'string') {
    return { languageCode: NEXT_PUBLIC_DEFAULT_LANGUAGE, scriptCode: undefined, countryCode: undefined }
  }

  const match = locale.match(localesRegExp)

  if (!match) {
    return { languageCode: locale, scriptCode: undefined, countryCode: undefined }
  }

  const [_locale, languageCode, scriptCode, countryCode] = match
  return { languageCode, scriptCode, countryCode }
}

export function changeLanguage(localeCode: string | string[] | undefined, newLanguageCode: string) {
...
export function switchLocale(asPath: string, newLocaleCode: string): string {
  ...
  const localeRegex = new RegExp(`^/${localesRegExpAsString}`)

  if (localeRegex.test(asPath)) {
    return asPath.replace(localeRegex, `/${newLocaleCode}`)
  }

  return `/${newLocaleCode}${asPath.startsWith('/') ? '' : '/'}${asPath}`
}
