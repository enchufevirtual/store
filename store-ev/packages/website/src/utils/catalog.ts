...
const ROOT_NAVIGATION_TAXON_IDS = new Set([
  'taxon_1',
  'taxon_3',
  'taxon_4',
  'taxon_5'
...
const ROOT_NAVIGATION_LABELS: Record<string, string> = {
  taxon_1: 'Accesorios',
  taxon_3: 'Ush Flash memory',
  taxon_4: 'discos duros',
  taxon_5: 'otros'
}

export const getRootNavigationLinks = (catalog: Unserializable<Catalog>): NavigationLink[] => {
  return getPrimaryTaxonomy(catalog).taxons
    .filter(taxon => ROOT_NAVIGATION_TAXON_IDS.has(taxon.id))
    .map(taxon => ({
      key: taxon.id,
      href: getSearchUrl(taxon),
      text: ROOT_NAVIGATION_LABELS[taxon.id] ?? taxon.label,
